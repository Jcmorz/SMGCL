...
class DRDataset():
    def __init__(self, dataset_name="Fdataset",
                 drug_neighbor_num=10,
                 disease_neighbor_num=10):

        assert dataset_name in ["Fdataset", "Cdataset", "DNdataset", "lrssl", "hdvd", "lagcn"]   # assert(断言)用于判断一个表达式，在表达式条件为 false 的时候触发异常。

        self.dataset_name = dataset_name

        if dataset_name=="lrssl":
            source_data = load_DRIMC(name=dataset_name)
        elif dataset_name=="hdvd":
            source_data = load_HDVD()
        elif dataset_name=="lagcn":
            source_data = load_LAGCN()
        else:
            source_data = scio.loadmat(f"dataset/{dataset_name}.mat")



        # 查看mat文件数据
        # print(type(source_data))    # 查看mat文件数据的格式
        # print(source_data.keys())
        #
        # data_association = np.array(np.nonzero(source_data['didr'])).T
        # print(type(data_association))
        #
        # np.savetxt('case.csv', data_association, delimiter=',')
        #
        # with open('drug_ref.csv', 'w', newline='') as file:
        #     mywriter = csv.writer(file, delimiter=',')
        #     mywriter.writerows(source_data['Wrname'])
        #
        # with open('disease_ref.csv', 'w', newline='') as file:
        #     mywriter = csv.writer(file, delimiter=',')
        #     mywriter.writerows(source_data['Wdname'])



        self.drug_sim = source_data["drug"].astype(np.float)
        self.disease_sim = source_data["disease"].astype(np.float)

        self.drug_name = source_data["Wrname"].reshape(-1)
        self.drug_num = len(self.drug_name)

        self.disease_name = source_data["Wdname"].reshape(-1)
        self.disease_num = len(self.disease_name)

        self.interactions = source_data["didr"].T
    ...
    def add_argparse_args(parent_parser):
        parser = parent_parser.add_argument_group("DDAs dataset config")

        parser.add_argument("--dataset_name", default="Fdataset",
                            choices=["Fdataset", "Cdataset", "DNdataset", "lrssl", "hdvd"])

        parser.add_argument("--drug_neighbor_num", default=4, type=int,
                            help="Number of neighbors of nodes when constructing drug similarity graph.")

        parser.add_argument("--disease_neighbor_num", default=4, type=int,
                            help="Number of neighbors of nodes when constructing disease similarity graph.")
...
class Dataset():
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", **kwargs):
        ...
        row, col = np.nonzero(mask&dataset.interactions.astype(bool))   # Return the indices of the elements that are non-zero.
        ...
        if not fill_unkown:
            ...
            row_idx, col_idx = torch.meshgrid(torch.arange(mask.shape[0]), torch.arange(mask.shape[1]))     # indexing – (str, optional): the indexing mode, either “xy” or “ij”, defaults to “ij”. If “ij” is selected, the dimensions are in the same order as the cardinality of the inputs.
            self.interaction_edge = torch.stack([row_idx.reshape(-1), col_idx.reshape(-1)])     # {Tensor: (2, drug_num * disease_num)}
            self.label = torch.clone(torch.from_numpy(dataset.interactions)).float()
            self.label[~mask] = 0      # ~ 表示非操作；
            self.valid_mask = torch.from_numpy(mask)
            self.matrix_mask = torch.from_numpy(mask)

        self.drug_edge = dataset.drug_edge          # drug_edge是由药物相似度构建的相似度图
        self.disease_edge = dataset.disease_edge    # disease_edge是由疾病相似度构建的相似度图
...
class GraphDataIterator(DataLoader):
    def __init__(self, dataset, mask, fill_unkown=True, stage="train", batch_size=1024*4, shuffle=False,
        ...
        if shuffle and stage=="train":
            ...
            sampler = SequentialSampler(dataset)    # 按顺序对元素进行采样。
...
class CVDataset(pl.LightningDataModule):
    ...
    def add_argparse_args(parent_parser):
        ...
        parser.add_argument("--seed", default=918, type=int)
        return parent_parser
