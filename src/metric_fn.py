...
def get_metrics(real_score, predict_score):
    ...
    sorted_predict_score = np.array(
        sorted(list(set(np.array(predict_score).flatten()))))       # set()函数创建一个无序不重复元素集；list()函数可以将任何可迭代数据转换为列表类型，并返回转换后的列表。
    sorted_predict_score_num = len(sorted_predict_score)
    thresholds = sorted_predict_score[np.int32(
        sorted_predict_score_num*np.arange(1, 70)/70)]
    thresholds = np.mat(thresholds)
    thresholds_num = thresholds.shape[1]

    predict_score_matrix = np.tile(predict_score, (thresholds_num, 1))  # tile(A, reps): Construct an array by repeating A the number of times given by reps.
